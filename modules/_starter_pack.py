...
def _fetch_inbox(dot_email: str, app_pswd: str, max_emails: int = 5) -> list:
    """
    Fetch unread emails from Gmail via IMAP using app password.
    ...
    try:
        ...
        mail.login(dot_email, app_pswd)
        mail.select("inbox")
        _, ids = mail.search(None, "UNSEEN")
        email_ids = ids[0].split()[-max_emails:]
        results = []
