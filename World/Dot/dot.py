...
def init_world():
    ...
    for d in directories:
        if not d.exists():
            ...
            log.info(f"--  Dot built directory: {d.relative_to(ROOT)}")
...
def write_motion(content: str):
    ...
    header = f"# Letter from Dot -- {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n\n"
...
def dispatch_email() -> str:
    ...
    if not recipient.get("found") or not recipient.get("email") or recipient.get("confidence", 0) < 9:
        ...
        return (
            f"### Email Dispatch -- Verification REJECTED \n\n"
    ...
    try:
        ...
        for uid in ids[-10:]:   # cap at 10 most recent
            ...
            if any(x in sender.lower() for x in ["mailer-daemon", "postmaster", "undeliverable"]):
                summaries.append(f"[!] BOUNCE: {subject} -- {sender}")
            else:
