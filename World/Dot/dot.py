"""
dot.py â€” Watchdog & Support Agent
Project Sam-and-dot
...
What Dot does each run:
  1.  Wisdom check   â€” evaluate sam.py against wisdom.txt, write motion.md
  2.  Experiences    â€” curate experiences.json (keep/consolidate/forget)
  3.  Email dispatch â€” if request.json is pending, compose & send HTML email
  4.  Bag excavation â€” rehabilitate broken experiments in bag/
  5.  Sunday only    â€” check inbox for replies, summarise to motion.md
"""
...
# Add Dot and Sam to path for imports
if str(DOT_DIR) not in sys.path:
    sys.path.insert(0, str(DOT_DIR))
...
sh.setFormatter(logging.Formatter("%(asctime)s [DOT][%(levelname)s] %(message)s"))
log.addHandler(sh)

# JSON File Handler
fh = logging.handlers.RotatingFileHandler(BAG / "dot.log", maxBytes=500_000, backupCount=3)
fh.setFormatter(JSONFormatter())
log.addHandler(fh)

# â”€â”€ Gemini client (Dot's OWN independent instance) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Directory Guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def init_world():
    ...
    for d in directories:
        if not d.exists():
            ...
            log.info(f"ðŸ—ï¸  Dot built directory: {d.relative_to(ROOT)}")
...
def _dot_gemini_base_call(prompt: str, temperature: float):
...
def ask_gemini(prompt: str, bypass_cache: bool = False, temperature: float = 0.2, expect_json: bool = False) -> str:
    from Others.semantic_cache import check_cache, update_cache, get_db
    global _CALL_DELAY

    get_db()
    # For Dot, we use a more granular cycle to allow multiple runs per day
    # or explicitly bypass cache for stateful tasks.
    cycle = int(datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%d%H")) # Hourly cycle
    ...
    try:
        ...
        response = _dot_gemini_base_call(prompt, temperature)
        ...
        if is_truncated:
             ...
             # Basic retry for truncation
             response = _dot_gemini_base_call(prompt + " (continue)", temperature)
             res = validate_gemini_response(response, expected_type='text')

        update_cache(prompt, res, cycle)
        return res
    except Exception as e:
        log.error(f"Dot Gemini error after resilience: {e}")
        return f"[Dot Gemini error: {e}]"
...
def read_sam_alerts() -> str:
    ...
    for alert in alerts:
        ...
        # Archive alert to Dot's Memory â€” ensure directory exists
...
def append_sent_log(entry: dict):
    ...
    log_data = log_data[-100:]  # keep last 100 sent emails

    import tempfile
    sent_path = _bag_data("sent_emails")
    tmp_path = None
    try:
        tmp_fd, tmp_path = tempfile.mkstemp(dir=str(sent_path.parent), suffix=".tmp")
        with os.fdopen(tmp_fd, 'w') as f:
            json.dump(log_data, f, indent=2)
        os.replace(tmp_path, str(sent_path))
    except Exception as e:
        if tmp_path and os.path.exists(tmp_path):
            try: os.unlink(tmp_path)
            except Exception as e: pass
...
def write_motion(content: str):
    ...
    header = f"# Letter from Dot â€” {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n\n"
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TASK 1 â€” WISDOM CHECK
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def wisdom_check() -> str:
    log.info("â”€â”€ Task 1: Wisdom Check â”€â”€")
    wisdom  = load_wisdom()
    sam_src = load_sam_py()
    ...
    # Use a content hash of sam.py for smarter caching (Issue #9)
    import hashlib
    sam_hash = hashlib.md5(sam_src.encode()).hexdigest()[:8]
    cache_key_prefix = f"[wisdom_check sam_hash={sam_hash}]"
    ...
    prompt = (
        ...
        f"=== SAM'S PERSONALITY ===\n{personality}\n\n"
        f"=== SAM'S CURRENT CODE (sam.py â€” full source) ===\n{sam_src}\n\n"
        ...
        "2. Positive highlights â€” things Sam did well this cycle.\n"
    ...
    findings = ask_gemini(cache_key_prefix + "\n\n" + prompt)
    log.info("Wisdom check complete.")
    return findings


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TASK 2 â€” EXPERIENCES CURATION
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def curate_experiences() -> str:
    log.info("â”€â”€ Task 2: Experiences Curation â”€â”€")
    experiences = load_experiences()

    if not experiences:
        log.info("No experiences to curate yet.")
        return "(No experiences to curate yet â€” Sam hasn't completed a full cycle.)"

    _sleep()
    prompt = (
        "You are Dot, Sam's memory curator. Below is Sam's experiences.json â€” "
        ...
        "Respond ONLY with valid JSON, no markdown code fences or explanation.\n"
        "Schema Example:\n"
        "{\n"
        "  \"keep\": [1, 2],\n"
        "  \"forget\": [3],\n"
        "  \"consolidated\": [{\"cycle\": 4, \"consolidated_from\": [5, 6], ...}],\n"
        "  \"summary\": \"...\"\n"
        "}\n\n"
        "Be conservative â€” when in doubt, keep. Only forget truly redundant or outdated entries."
    )
    raw = ask_gemini(prompt, bypass_cache=True, expect_json=True)

    def parse_curation_result(raw_text: str) -> dict:
        """Parse curation response with multiple fallback strategies."""
        ...
        log.error(f"Failed to parse curation JSON. Raw response:\n{raw_text}")
        return {"keep": [], "forget": [], "consolidated": [], "summary": "Failed to parse curation."}

    curation = parse_curation_result(raw)

    # Validate keys exist
    for key in ["keep", "forget", "consolidated"]:
    ...
    def _extract_cycle(c):
        if isinstance(c, int): return c
        if isinstance(c, dict): return c.get("cycle")
        return None

    # Priority 2.4: Protect unmentioned entries
    mentioned = keep | forget | {
        _id for entry in consolidated
        for c in entry.get("consolidated_from", [])
        if (_id := _extract_cycle(c)) is not None
    }
    unmentioned = {e.get("cycle") for e in experiences if e.get("cycle")} - mentioned
    if unmentioned:
        log.warning(f"Curation: {len(unmentioned)} entries not mentioned by Gemini â€” keeping them: {sorted(unmentioned)}")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TASK 3 â€” EMAIL DISPATCH
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def dispatch_email() -> str:
    log.info("â”€â”€ Task 3: Email Dispatch â”€â”€")

    EMAIL_ADDR = os.environ.get("EMAIL", "")
    APP_PSWD   = os.environ.get("APP_PSWD", "")

    if not EMAIL_ADDR or not APP_PSWD:
        log.warning("EMAIL or APP_PSWD secrets not set â€” skipping email dispatch.")
    ...
    if not recipient.get("found") or not recipient.get("email") or recipient.get("confidence", 0) < 9:
        ...
        return (
            f"### Email Dispatch â€” Verification REJECTED ðŸ›¡ï¸\n\n"
    ...
    compose_prompt = (
        ...
        f"- Be concise â€” no more than 180 words in the body.\n"
        ...
        f"- Sign off as 'Sam' â€” mention it is an autonomous developer agent briefly and naturally,\n"
        f"  not as a disclaimer but as an interesting detail.\n\n"
        f"Respond ONLY with a JSON object:\n"
        f"  - 'subject': a specific subject line (not generic â€” reference their actual work)\n"
    ...
    if success:
        ...
        return "(Email send failed â€” request kept pending for next run.)"


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TASK 4 â€” BAG EXCAVATION
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def excavate_bag() -> str:
    log.info("â”€â”€ Task 4: Bag Review â”€â”€")

    # Core files Dot never questions â€” infrastructure, not experiments
    DOT_PROTECTED = {
    ...
    prompt = (
        ...
        "and give Sam a clear, honest recommendation: keep or delete â€” and why.\n\n"
        "For each file below:\n"
        ...
        "- **filename.py** â€” [one-sentence description] â†’ **KEEP** / **DELETE**: [reason]"
    )
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# TASK 5 â€” SUNDAY INBOX CHECK (runs only on Sundays)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def sunday_inbox_check() -> str:
    log.info("â”€â”€ Task 5: Sunday Inbox Check â”€â”€")

    EMAIL_ADDR = os.environ.get("EMAIL", "")
    APP_PSWD   = os.environ.get("APP_PSWD", "")

    if not EMAIL_ADDR or not APP_PSWD:
        log.warning("EMAIL or APP_PSWD not set â€” skipping inbox check.")
        return "(Inbox check skipped: credentials not configured.)"

    sent_log = load_sent_log()
    if not sent_log:
        log.info("No sent emails on record â€” nothing to check replies for.")
        return "(No sent emails on record yet â€” nothing to check for.)"
    ...
    try:
        ...
        for uid in ids[-10:]:   # cap at 10 most recent
            ...
            if any(x in sender.lower() for x in ["mailer-daemon", "postmaster", "undeliverable"]):
                summaries.append(f"âš ï¸ BOUNCE: {subject} â€” {sender}")
            else:
...
def run():
    ...
    log.info("  DOT â€” Daily Watchdog Run Starting")
    log.info("â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•")
    ...
    # Task 1: Wisdom check (always runs first â€” becomes the base of motion.md)
    ...
    if today.weekday() == 6:   # 6 = Sunday
        ...
        log.info(f"Today is {today.strftime('%A')} â€” inbox check reserved for Sunday.")
    ...
    if today.weekday() == 6: # Sunday
        try:
            ...
            if topic and "error" not in topic.lower():
                ...
                # Atomic write for goals.json in Dot
                import tempfile
                tmp_path = None
                try:
                    ...
                    log.error(f"Failed to save goals in Dot: {e}")
        except Exception as e:
    ...
    try:
        ...
        if stale:
            append_motion("Worklog â€” Stale Items", stale)
    ...
    send_html_email(
      ...
      subject=f"Dot's Daily Report â€” {datetime.date.today()}",
