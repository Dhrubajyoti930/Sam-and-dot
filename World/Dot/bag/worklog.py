"""
worklog.py â€” Sam's wandering notebook.
...
def _worklog_path() -> Path:
    """Read-only access for Dot to Sam's worklog."""
    # Dot reads Sam's worklog â€” it lives in Sam's My_memories
    return Path(__file__).parent.parent.parent / "Sam" / "My_memories" / "worklog.json"
...
def stale_report(current_cycle: int) -> str:
    ...
    if not stale:
        return ""
    lines = [f"### Worklog â€” {len(stale)} stale item(s) for Sam\n"]
    for e in stale:
