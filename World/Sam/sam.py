...
def self_check() -> bool:
    """Rigorous integrity check -- returns False on failure. Rollback is handled by caller."""
...
def phase_vii_state_saving(goals: dict, skill: str, idea: str, plan: str, evolution: str, motion_content: str):
    ...
    if new_who_text == WHO_I_AM.read_text():
         log.warning("WHO_I_AM.md regex substitution made no changes -- check template.")
...
def run_cycle():
    ...
    if not (self_check() and behaviour_check()):
        ...
        log.info("  Initiating automatic repair sequence...")
        repair_bag_modules()
        if not (self_check() and behaviour_check()):
            log.critical("  Automatic repair failed. Aborting cycle for owner safety.")
            _bag_data("cycle_status").write_text("broken")
            return
