...
PHASE_IV_PROMPT = (
    ...
    "Format as a short markdown document with: ## Idea, ## Why, ## Implementation Steps, ## Risk, ## Rollback Strategy, ## Time-to-Detection.\n"
    "Be critical — question the idea yourself, identify one potential failure mode, propose a mitigation strategy, define a 'Detection Mechanism' to catch this failure in production, and assign a confidence score (1-10) to the implementation's success probability, and estimate the 'Severity Impact' (1-10) of the failure mode if it occurs."
)
