"""
bag/workshop.py -- Sam's craft organization under workshop_bench/
"""
...
def format_layout_for_prompt(workshop: Path) -> str:
    ...
    if not folders:
        return f"  (no folders yet in {WORKSHOP_ROOT}/ -- you may create some)\n"
...
def apply_workshop_moves(workshop: Path, moves: list, log) -> tuple[list[str], dict[str, str]]:
    ...
    for op in moves or []:
        ...
        log.info(f"Workshop move: {WORKSHOP_ROOT}/{src_rel} -> {WORKSHOP_ROOT}/{dst_rel}")
...
def finalize_workshop_moves(sam_root: Path, workshop: Path, bag: Path, path_map: dict[str, str], log) -> bool:
    ...
    log.error("Workshop moves broke repo integrity -- rolling back.")
