...
def trial_and_verdict(plan_ops, root: Path, test_script: Path):
    ...
    log.info("[=] Entering Trial Mode for proposed changes...")
    ...
    if not success:
        log.error("[X] Verdict: REJECTED (Patch Application Failed)")
    ...
    try:
        ...
        if result.returncode != 0:
            log.error(f"[X] Verdict: REJECTED (Syntax/Logic Violation)\n{result.stdout}")
            return False, f"Syntax Gate Failed: {result.stdout[:200]}"
    except Exception as e:
        log.warning(f"[!] Syntax Gate Bypassed (Ruff error: {e})")
    ...
    if test_script.exists():
        ...
        try:
            ...
            if result.returncode != 0:
                log.error(f"[X] Verdict: REJECTED (Behavioural Failure)\n{result.stdout}")
                return False, f"Behavioural Gate Failed: {result.stderr[:200]}"
        except Exception as e:
            log.error(f"[X] Verdict: REJECTED (Test Runner Crashed: {e})")
            return False, f"Test Runner Crash: {e}"
    else:
        log.info(" No test script found. Behavioural Gate skipped.")

    log.info("[ok] Verdict: ACCEPTED. Changes merged into World state.")
    return True, "Success"
