...
def load_callable(workshop: Path, module_basename: str, attr: str, default=None):
    ...
    # ── GENEROUS RESOLVE ──
    # Check workshop bench first, then fall back to bag and Sam root
    path = find_workshop_py(workshop, module_basename)
    if not path:
        # Fallback 1: Check bag/
        bag_path = workshop.parent / "bag" / f"{module_basename}.py"
        if bag_path.exists():
            path = bag_path
        else:
            # Fallback 2: Check Sam root
            root_path = workshop.parent / f"{module_basename}.py"
            if root_path.exists():
                path = root_path
    ...
    if not path:
        if default is not None:
            return default
        raise ImportError(f"No workshop or bag module '{module_basename}' found.")
